...
def convert_track(start = 0, length = 7, output_name='default', output_volume=0.06):
    ...
    command = f"ffmpeg -ss {start} -t {length} -i soundboard/staging/staging.mp3 -vn -ar 44100 -ac 2 -b:a 192k -filter:a \"volume={output_volume}\" -y soundboard/{output_name}.mp3"
