...
microphone_name = 'Samson Q2U (Cmd + M)'
...
async def autotune_request(voice_effect='AutoPitch'):
    ...
    data = {'sourceName': microphone_name, 'filterName': voice_effect, 'filterEnabled': True}
    ...
    data = {'sourceName': microphone_name, 'filterName': 'AutoPitch', 'filterEnabled': False}
    result = await ws.call('SetSourceFilterVisibility', data)

    data2 = {'scene-name': 'Audio and Effects Scene', 'item': 'AutoPitch Picture', 'visible': False}
    result2 =  await ws.call('SetSceneItemProperties', data2)

    data3 = {'sourceName': microphone_name, 'filterName': 'Bane', 'filterEnabled': False}
